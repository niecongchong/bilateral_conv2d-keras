...
# def bilateral_conv2d(inputs, batch_size, kernel, kernel_size, strides, output_shape, data_format=None):
#     """Apply 2D conv with un-shared weights.
#
#     # Arguments
#         inputs: 4D tensor with shape:
...
#                 (batch_size, new_rows, new_cols, filters)
#                 if data_format='channels_last'.
#         kernel: the shared weight for convolution,
#                 with shape (kernel_row, kernel_col, input_filter, filters)
#         kernel_size: a tuple of 2 integers, specifying the
#                      width and height of the 2D convolution window.
#         strides: a tuple of 2 integers, specifying the strides
#                  of the convolution along the width and height.
...
#             slice_col = py_slice(j * stride_col,
#                                  j * stride_col + kernel_size[1])
#             center_row = i * stride_row + tf.cast((kernel_size[0]-1) / 2, tf.int32)
#             center_col = i * stride_row + tf.cast((kernel_size[1]-1) / 2, tf.int32)
#             if data_format == 'channels_first':
#                 xs.append(reshape(inputs[:, :, slice_row, slice_col],
#                                   (1, -1, kernel_row * kernel_col * input_filter)))
#             else:
#                 xs.append(reshape(inputs[:, slice_row, slice_col, :],
#                                   (1, -1, kernel_row * kernel_col * input_filter)))
#
#                 patch = reshape(inputs[:, slice_row, slice_col, :], (-1, kernel_row * kernel_col, input_filter))
#                 center = reshape(inputs[:, center_row, center_col, :], (-1, 1, input_filter))
#
#                 SqA = tf.square(patch)
#                 sumSqA = tf.reduce_sum(SqA, axis=-1, keepdims=True)
#
#                 SqB = tf.square(center)
#                 sumSqB = tf.reduce_sum(SqB, axis=-1, keepdims=True)
#                 sumSqBEx = tf.tile(sumSqB, (1, kernel_row * kernel_col, 1))
#
#                 ABT = K.batch_dot(patch, tf.transpose(center, (0, 2, 1)))
#
#                 SqED = sumSqBEx + sumSqA - 2 * ABT
#
#                 ED = tf.sqrt(SqED)
#
#                 coefficient_weight = tf.exp(-ED/50)
#
#                 coefficient_weight = reshape(coefficient_weight, (1, -1, kernel_row, kernel_col, 1, 1))
#                 coefficient_weight = tf.tile(coefficient_weight, (1, 1, 1, 1, input_filter, filters))
#                 coefficient_weight = reshape(coefficient_weight, (1, -1, kernel_row * kernel_col * input_filter, filters))
#                 coefficient_weights.append(coefficient_weight)
#
#     x_aggregate = concatenate(xs, axis=0)
#     x_aggregate = reshape(x_aggregate, (-1, kernel_row * kernel_col * input_filter))
#
#     coefficient_aggregate = concatenate(coefficient_weights, axis=0)
#     coefficient_aggregate = reshape(coefficient_aggregate, (-1, kernel_row * kernel_col * input_filter, filters))
#
#     kernel_weights = reshape(kernel, (1, 1, kernel_row, kernel_col, input_filter, filters))
#     kernel_weights = tf.tile(kernel_weights, (output_row*output_col, batch_size, 1, 1, 1, 1))
#     kernel_weights = reshape(kernel_weights, (-1, kernel_row * kernel_col * input_filter, filters))
#
#     weights = coefficient_aggregate * kernel_weights
#
#     output = batch_dot(x_aggregate, weights)
#     output = reshape(output,
#                      (output_row, output_col, -1, filters))
#
#     if data_format == 'channels_first':
...
def bilateral_conv2d(inputs, kernel, strides, output_shape, sigma=50):
    ...
    ED = SqED  # tf.sqrt(SqED)

    coefficient_weight = tf.exp(-ED/(2*sigma**2))  # shape=(b * output_row * output_col, kernel_row * kernel_col, 1)
    ...
    output = batch_dot(image_patches, weights)
